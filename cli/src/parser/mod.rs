...
pub fn parse<'a>(args: &'a ArgMatches<'_>) -> Service<'a> {
    if args.is_present("didkey") {
        return Service::DIDKey(didkey::parse(&args.subcommand_matches("didkey").expect("Error parsing request")));
    }
    
    else if args.is_present("didcomm") {
        return Service::DIDComm(didcomm::parse(&args.subcommand_matches("didcomm").expect("Error parsing request")));
    }

    else if args.is_present("wallet") {
        return Service::Wallet(wallet::parse(&args.subcommand_matches("wallet").expect("Error parsing request")));
    }
