...
fn pack(args: &PackArgs) {
    let sender_key : JsonWebKey = match args.sender_key {
        Some(input) => {
            ...
            file.read_to_string(&mut key).expect("Unable to read sender_key file");
    ...
    let receiver_key : JsonWebKey = match args.receiver_key {
        Some(input) => {
            ...
            let mut key = String::new();
            file.read_to_string(&mut key).expect("Unable to read receiver_key file");
    ...
    let associated_data = match args.associated_data {
        Some(input) => {
            ...
            file.read_to_end(&mut buffer).expect("Unable to read associated data file");
            buffer
        },
        None => vec![],
    };
    ...
    let plaintext = match args.plaintext {
        Some(input) => {
            ...
            let mut key = String::new();
            file.read_to_string(&mut key).expect("Unable to read plaintext file");
            key.as_bytes().to_vec()
        },
        None => {
    ...
    match args.out {
        ...
        None => std::io::stdout().write_all(&res.message.expect("Malformed pack response").to_vec()).expect("Unable to write to stdout"),
    };
}
...
fn unpack(args: &UnpackArgs) {
    ...
    let encrypted_message : EncryptedMessage = match args.encrypted_message {
        Some(input) => {
            ...
            file.read_to_end(&mut message).expect("Unable to read encrypted_message file");
            EncryptedMessage::from_vec(&message).expect("Invalid encrypted message")
        },
        None => {
            let mut message : Vec::<u8> = Vec::new();
            std::io::stdin().read_to_end(&mut message).expect("Unable to read from stdin");
            EncryptedMessage::from_vec(&message).expect("Invalid encrypted message")
    ...
    let message = std::str::from_utf8(&res.plaintext).expect("Unable to convert bytes to text").to_string();
    println!("{}", message);
}
...
fn sign(args: &SignArgs) {
    let key : JsonWebKey = match args.key {
        Some(input) => {
            ...
            let mut key = String::new();
            file.read_to_string(&mut key).expect("Unable to read key file");
    ...
    let payload = match args.payload {
        Some(input) => {
            ...
            file.read_to_string(&mut payload).expect("unable to read payload file");
            payload.as_bytes().to_vec()
        },
        None => {
            let mut payload = String::new();
            std::io::stdin().read_to_string(&mut payload).expect("Unable to read from stdin");
            payload.as_bytes().to_vec()
        }
    };
    ...
    match args.out {
        ...
        None => std::io::stdout().write_all(&res.message.expect("Malformed response").to_vec()).expect("Unable to write to stdout"),
    };
}
...
fn verify(args: &VerifyArgs) {
    ...
    let signed_message : SignedMessage = match args.signed_message {
        Some(input) => {
            ...
            let mut message : Vec::<u8> = Vec::new();
            file.read_to_end(&mut message).expect("Unable to read signed message file");
