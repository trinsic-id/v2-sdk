...
export type IDVSessionState = typeof IDVSessionState[keyof typeof IDVSessionState];
...
export function instanceOfIDVSessionState(value: any): boolean {
    for (const key in IDVSessionState) {
        if (Object.prototype.hasOwnProperty.call(IDVSessionState, key)) {
            if ((IDVSessionState as Record<string, IDVSessionState>)[key] === value) {
                return true;
            }
        }
    }
...
export function IDVSessionStateFromJSONTyped(json: any, ignoreDiscriminator: boolean): IDVSessionState {
