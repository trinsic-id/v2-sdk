...
class AccountService extends ServiceBase {
  ...
  Future<String> signIn(SignInRequest? request) async {
    ...
    request.ecosystemId = request.ecosystemId != "" ? request.ecosystemId : serviceOptions.defaultEcosystem;
    SignInResponse response = await this.client.signIn(request);
    var authToken = Base64Encoder.urlSafe().convert(response.profile.writeToBuffer());
    serviceOptions.authToken = authToken;
    return authToken;
  }

  static AccountProfile unprotectProfile(AccountProfile profile, String securityCode) {
  ...
  static String unprotect(String base64Profile, String securityCode) {
    var accountProfile = AccountProfile.fromBuffer(Base64Decoder().convert(base64Profile));
  ...
  static AccountProfile protectProfile(AccountProfile profile, String securityCode) {
    var utf8code = Uint8List.fromList(utf8.encode(securityCode));
  ...
  Future<InfoResponse> listDevices() async {

  }

  Future<InfoResponse> revokeDevice() async {
