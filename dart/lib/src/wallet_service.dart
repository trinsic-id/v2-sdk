...
// target: /home/runner/work/sdk/sdk/dart/lib/src/wallet_service.dart
  ...
  Future<ListByVerificationTemplateResponse> listByVerificationTemplate(
      ListByVerificationTemplateRequest request) async {
    ///  List credentials which match a given verification template

    return client.listByVerificationTemplate(request,
        options: await buildMetadata(request: request));
  }

  Future<CreateDidWebDocResponse> createDidWebDoc(
      CreateDidWebDocRequest request) async {
    ///  Create a `did:web` document from a wallet's key(s)

    return client.createDidWebDoc(request,
        options: await buildMetadata(request: request));
  }
