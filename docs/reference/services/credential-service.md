...
Furthermore, the "Create Offer" endpoint enables the issuing party to define and configure the holder binding properties of the credential. This means that the issued VC will be bound to the intended holder, making it non-transferable and restricted for use solely by the authorized holder. If {{ field_ref("CreateCredentialOfferRequest", "holder_binding") }} is set to true , the credential that is issued will have `credentialSubject.id` set to the holder's wallet DID - otherwise, it'll be a static value `urn:vc:subject:0`. During proof creation, if the VC has a holder DID in the field `credentialSubject`, in the resulting proof it'll also be appended a _proof of ownership_. This will show to the verifier that whoever is presenting the proof really owns the DID set by the issuer in the `credentialSubject` field of the credential.
