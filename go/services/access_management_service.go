...
func NewAccessManagementService(options *Options) (AccessManagementService, error) {
	...
	service := &accessBase{
		Service: base,
		client:  provider.NewAccessManagmentClient(base.GetChannel()),
	}
...
type AccessManagementService interface {
	...
	// RemoveRoleAssignment  Removes a role assignment from the account
	...
	// ListRoleAssignments  List the role assignments for the given account
...
type accessBase struct {
	Service
	client provider.AccessManagmentClient
}
...
// AddRoleAssignment  Adds a role assignment to an account
func (p *providerBase) AddRoleAssignment(userContext context.Context, request *provider.AddRoleAssignmentRequest) (*provider.AddRoleAssignmentResponse, error) {
...
// RemoveRoleAssignment  Removes a role assignment from the account
func (p *providerBase) RemoveRoleAssignment(userContext context.Context, request *provider.RemoveRoleAssignmentRequest) (*provider.RemoveRoleAssignmentResponse, error) {
	md, err := p.GetMetadataContext(userContext, request)
	if err != nil {
		return nil, err
	}
...
// ListRoleAssignments  List the role assignments for the given account
func (p *providerBase) ListRoleAssignments(userContext context.Context, request *provider.ListRoleAssignmentsRequest) (*provider.ListRoleAssignmentsResponse, error) {
	md, err := p.GetMetadataContext(userContext, request)
	if err != nil {
		return nil, err
	}
