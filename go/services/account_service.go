...
// target: ..\sdk\go\services\account_service.go
    ...
    // SignIn  Sign in to an already existing account 
    SignIn(userContext context.Context, request *account.SignInRequest) (*account.SignInResponse, error)
    // Login  Login to account. If account doesn't exist, new will be created 
    Login(userContext context.Context, request *account.LoginRequest) (*account.LoginResponse, error)
    // LoginConfirm  Confirm login step by responding to the challenge request 
...
// SignIn  Sign in to an already existing account
func (a *accountBase) SignIn(userContext context.Context, request *account.SignInRequest) (*account.SignInResponse, error) {
    // TODO - Handle a flag for the metadata context
	md, err := a.GetMetadataContext(userContext, request)
    if err != nil {
        return nil, err
    }
    response, err := a.client.SignIn(md, request)
    if err != nil {
		return nil, err
	}
	return response, nil
