...
func TestProviderBase_InviteParticipant(t *testing.T) {
	...
	t.Run("Contact method unset throws", func(t *testing.T) {
		providerService, err := NewProviderService(ServiceOptions{config: TrinsicTestConfig()})
		if !assert2.Nil(err) {
			return
		}
...
func TestProviderService_InviteParticipant(t *testing.T) {
	...
	providerService, err := NewProviderService(ServiceOptions{channel: accountService.GetChannel()})
	if !assert2.Nil(err) {
		return
	}
