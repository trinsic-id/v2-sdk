...
type WalletService interface {
	...
	// CreateDidWebDoc  Create a `did:web` document from a wallet's key(s)
	CreateDidWebDoc(userContext context.Context, request *wallet.CreateDidWebDocRequest) (*wallet.CreateDidWebDocResponse, error)
...
// CreateDidWebDoc  Create a `did:web` document from a wallet's key(s)
func (w *universalWalletBase) CreateDidWebDoc(userContext context.Context, request *wallet.CreateDidWebDocRequest) (*wallet.CreateDidWebDocResponse, error) {
	md, err := w.GetMetadataContext(userContext, request)
	if err != nil {
		return nil, err
	}
	response, err := w.client.CreateDidWebDoc(md, request)
	if err != nil {
		return nil, err
	}
	return response, nil
