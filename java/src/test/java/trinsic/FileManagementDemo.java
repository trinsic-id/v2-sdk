...
public class FileManagementDemo {
  ...
  public static void run()
    ...
    var uploadResponse = trinsic.fileManagement().uploadFile(
        UploadFileRequest
            .newBuilder()
    ...
    // getFile() {
    var getFileResponse = trinsic.fileManagement().getFile(
        GetFileRequest
            .newBuilder()
            .setId(fileId)
            .build()
    ...
    // listFiles() {
    var listFilesResponse = trinsic.fileManagement().listFiles(
        ListFilesRequest
            .newBuilder()
    ...
    Assertions.assertEquals(listFilesResponse.getFiles(0).getId(), uploadResponse.getUploadedFile().getId());

    // getStorageStats() {
    var getStatsResponse = trinsic.fileManagement().getStorageStats(
        GetStorageStatsRequest.newBuilder().build()
    ).get();
    //}

    Assertions.assertEquals(getStatsResponse.getStats().getNumFiles(), 1);
    Assertions.assertEquals(getStatsResponse.getStats().getTotalSize(), uploadResponse.getUploadedFile().getSize());

    // deleteFile() {
    trinsic.fileManagement().deleteFile(
        DeleteFileRequest
            .newBuilder()
            .setId(fileId)
            .build()
