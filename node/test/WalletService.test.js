...
const { WalletService, ProviderService, ServerConfig, AccountService, CredentialService } = require("../lib");
...
describe("sdad", () => {
  test("create new account", async (t) => {
    ...
    let profile = await service.signIn();
  ...
  test("Demo: create wallet, set profile, search records, issue credential", async (t) => {
    let accountService = new AccountService(null, config);
    let profile = await accountService.signIn();
