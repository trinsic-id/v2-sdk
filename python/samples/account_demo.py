...
async def account_demo():
    ...
    # loginRequest() {
    login_response = await trinsic.account.login(request=LoginRequest(email="bob@example.com"))
    # }

    # We expect this to fail because this is a fake auth code
    try:
        # loginConfirm() {
        auth_token = await trinsic.account.login_confirm(challenge=login_response.challenge, auth_code="12345")
        # }
