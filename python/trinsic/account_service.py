...
class AccountService(ServiceBase):
    ...
    async def sign_in(self, *, request: SignInRequest = None) -> str:
        ...
        response = await self.client.sign_in(sign_in_request=request, metadata=self.build_metadata())
    ...
    async def login_confirm(self, *, challenge: bytes, auth_code: str) -> str:
        ...
        response = await self.client.login_confirm(request, metadata=self.build_metadata())
...
# target: ..\sdk\python\trinsic\account_service.py

    async def info(self, *, request: AccountInfoRequest) -> AccountInfoResponse:
        """
...
"""
        return await self.client.list_devices(request, metadata=self.build_metadata(request))
...
"""
        return await self.client.revoke_device(request, metadata=self.build_metadata(request))
...
"""
        return await self.client.authorize_webhook(request, metadata=self.build_metadata(request))
