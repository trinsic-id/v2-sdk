...
class ProviderService(ServiceBase):
    ...
    async def create_ecosystem(
        ...
        authenticate = len(request.name) > 0 or (request.details and len(request.details.email) > 0)
        metadata = self.build_metadata(request) if authenticate else self.build_metadata()
...
"""
        return await self.client.get_oberon_key(request, metadata=self.build_metadata(request))
