...
class ServiceBase(ABC):
    ...
    def stub_with_metadata(self, stub_type: Type["T"]) -> Type["T"]:
    ...
    def with_call_metadata(self, stub: ServiceStub) -> ServiceStub:
        ...
        async def wrapped_unary(this,
            ...
            await this._cls_unary_unary(route, request, response_type, timeout=timeout, deadline=deadline,
                                        metadata=metadata)

        setattr(stub, '_unary_unary', types.MethodType(wrapped_unary, stub))
        setattr(stub, '_cls_unary_unary', _cls_unary_unary)
        return stub
