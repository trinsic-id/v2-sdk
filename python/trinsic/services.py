...
class ServiceBase:
    ...
    def metadata(self) -> Metadata:
        ...
        return Metadata(("Capability-Invocation", self.cap_invocation))
    ...
    def set_profile(self, profile: WalletProfile):
        ...
        self.cap_invocation = base64.encodestring(bytes(proof_response.signed_document.to_json(), encoding="utf-8"))
...
class WalletService(ServiceBase):
    ...
    async def create_wallet(self, security_code: str = None) -> WalletProfile:
        ...
        my_did_document = my_key.did_document.to_dict()

        packed_message = DIDComm.pack(PackRequest(sender_key=my_exchange_key, receiver_key=provider_exchange_key,
                                                  plaintext=bytes(CreateWalletRequest(description="My Cloud Wallet",
                                                                                      controller=str(
                                                                                          my_did_document['id']),
        ...
        decrypted_response = DIDComm.unpack(UnpackRequest(message=response))
