...
def do_template(template_service)
  ...
  template.fields["firstName"] = Trinsic::Template_V1::TemplateField.new(:description => "Given name")
  template.fields["lastName"] = Trinsic::Template_V1::TemplateField.new()
  template.fields["batchNumber"] = Trinsic::Template_V1::TemplateField.new(:description => "")
  template.fields["countryOfVaccination"] = Trinsic::Template_V1::TemplateField.new(:description => "")
...
def vaccine_demo_run
  ...
  provider_service.ecosystem_id = account_service.ecosystem_id = ecosystem_id
  ...
  values = JSON.generate({"firstName": "Allison", "lastName": "Allisonne", "batchNumber": "123454321", "countryOfVaccination": "US"})

  # Issue credential
  issue_response = credential_service.issue_from_template(Trinsic::Credentials_V1::IssueFromTemplateRequest.new(:template_id => template.id, :values_json => values))
  ...
  begin
    # sendCredential() {
    send_response = credential_service.send(Trinsic::Credentials_V1::SendRequest.new(:document_json => credential, :email => "example@trinsic.id"))
    # }
  catch
  ...
  insert_response = wallet_service.insert_item(Trinsic::Wallet_V1::InsertItemRequest.new(:item_json => credential_service))
