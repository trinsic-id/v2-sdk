...
function handleCallback() {
    ...
    client
        ...
        .then(function (response) {
            ...
            document.getElementById("oidc-raw").innerText = JSON.stringify(
                response,
                null,
                2
            );
            // @ts-ignore
            document.getElementById("oidc-vp").innerText = JSON.stringify(
                response.vp_token,
                null,
                2
            );
            // @ts-ignore
            document.getElementById("oidc-credential").innerText =
                JSON.stringify(response.vp_token.credentialSubject, null, 2);

            //Register click event handler for copy buttons
            let copyButtons = document.getElementsByClassName("copy");
            for (let i = 0; i < copyButtons.length; i++) {
                copyButtons[i].addEventListener(
                    "click",
                    copy.bind(copyButtons[i])
                );
            }

            //Unhide the container for the VC data elements after they've been populated above
            document
                .getElementById("vc-info-container")
