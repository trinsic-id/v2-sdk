...
{
    $File = $Item.FullName
    &"$PROTOC" --plugin=import_style=typescript:protoc-gen-grpc-web=$PROTOC_GEN_TS_PATH --js_out=import_style=commonjs,binary:$OUTPUT_DIR --grpc-web_out=import_style=typescript,mode=grpcwebtext:$OUTPUT_DIR -I $PROTO_DIR $File
}
