...
{
    $File = $Item.FullName
    $Expr = "$PROTOC --plugin=protoc-gen-ts=$PROTOC_GEN_TS_PATH --ts_proto_out=$OUTPUT_DIR --ts_proto_opt=env=browser,esModuleInterop=true,outputServices=generic-definitions,useExactTypes=false,outputJsonMethods=false,exportCommonSymbols=false -I $PROTO_DIR $File"
    Write-Output $Expr
