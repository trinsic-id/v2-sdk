...
export class AccessManagementService extends ServiceBase {
    ...
    // target: C:\work\sdk\web\src\AccessManagementService.ts

    /** Adds a role assignment to an account */
    public async addRoleAssignment(
        request: proto.AddRoleAssignmentRequest
    ): Promise<proto.AddRoleAssignmentResponse> {
        return this.client.addRoleAssignment(request, {
            metadata: await this.buildMetadata(
                proto.AddRoleAssignmentRequest.encode(request).finish()
            ),
        });
    }
    /** Removes a role assignment from the account */
    public async removeRoleAssignment(
        request: proto.RemoveRoleAssignmentRequest
    ): Promise<proto.RemoveRoleAssignmentResponse> {
        return this.client.removeRoleAssignment(request, {
            metadata: await this.buildMetadata(
                proto.RemoveRoleAssignmentRequest.encode(request).finish()
            ),
        });
    }
    /** List the role assignments for the given account */
    public async listRoleAssignments(
        request: proto.ListRoleAssignmentsRequest
    ): Promise<proto.ListRoleAssignmentsResponse> {
        return this.client.listRoleAssignments(request, {
            metadata: await this.buildMetadata(
                proto.ListRoleAssignmentsRequest.encode(request).finish()
            ),
        });
    }
