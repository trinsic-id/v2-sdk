...
export class CredentialService extends ServiceBase {
    ...
    public async createCredentialOffer(
        request: proto.CreateCredentialOfferRequest,
    ): Promise<proto.CreateCredentialOfferResponse> {
        return this.client.createCredentialOffer(request, {
            metadata: await this.buildMetadata(
                proto.CreateCredentialOfferRequest.encode(request).finish(),
            ),
        });
    }
    /** Accept an offer to exchange a credential */
    public async acceptCredential(
        request: proto.AcceptCredentialRequest,
    ): Promise<proto.AcceptCredentialResponse> {
        return this.client.acceptCredential(request, {
            metadata: await this.buildMetadata(
                proto.AcceptCredentialRequest.encode(request).finish(),
            ),
        });
    }
    /** Reject an offer to exchange a credential */
    public async rejectCredential(
        request: proto.RejectCredentialRequest,
    ): Promise<proto.RejectCredentialResponse> {
        return this.client.rejectCredential(request, {
            metadata: await this.buildMetadata(
                proto.RejectCredentialRequest.encode(request).finish(),
            ),
        });
    }
