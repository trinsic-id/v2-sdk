...
export interface RetrieveDomainVerificationRecordResponse {
    ...
    verificationRecordName?: string | undefined;
...
export interface SearchWalletConfigurationResponse {
    ...
    results?: WalletConfiguration[] | undefined;
...
export interface WalletConfiguration {
     ...
     * @deprecated
     */
    externalIdentityIds?: string[] | undefined;
...
export interface UpgradeDidRequest {
    ...
    method?: SupportedDidMethod | undefined;
...
export const WalletConfiguration = {
    encode(
        ...
        if (
            message.externalIdentityIds !== undefined &&
            message.externalIdentityIds.length !== 0
        ) {
        ...
        while (reader.pos < end) {
            ...
            switch (tag >>> 3) {
                ...
                case 11:
                    ...
                    message.externalIdentities!.push(
                        WalletExternalIdentity.decode(reader, reader.uint32()),
                    );
                    continue;
            }
            if ((tag & 7) === 4 || tag === 0) {
    ...
    fromJSON(object: any): WalletConfiguration {
        return {
            ...
            externalIdentities: Array.isArray(object?.externalIdentities)
                ? object.externalIdentities.map((e: any) =>
                      WalletExternalIdentity.fromJSON(e),
                  )
                : [],
        };
    },
    ...
    toJSON(message: WalletConfiguration): unknown {
        ...
        if (message.externalIdentities?.length) {
            obj.externalIdentities = message.externalIdentities.map((e) =>
                WalletExternalIdentity.toJSON(e),
            );
        }
        return obj;
    },
    ...
    fromPartial(object: DeepPartial<WalletConfiguration>): WalletConfiguration {
        ...
        message.externalIdentityIds =
            object.externalIdentityIds?.map((e) => e) || [];
        message.ecosystemId = object.ecosystemId ?? "";
        message.description = object.description ?? "";
        message.externalIdentities =
            object.externalIdentities?.map((e) =>
                WalletExternalIdentity.fromPartial(e),
            ) || [];
        return message;
    },
};
...
export const WalletExternalIdentity = {
    ...
    fromPartial(
        object: DeepPartial<WalletExternalIdentity>,
    ): WalletExternalIdentity {
        const message = createBaseWalletExternalIdentity();
